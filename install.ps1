# Install the parcel command on Windows from a GitHub release:
#
#   powershell -ExecutionPolicy ByPass -c "irm https://github.com/griot-cloud/parcel/releases/latest/download/install.ps1 | iex"
#
# $env:PARCEL_VERSION picks a release (default: the latest), and $env:PARCEL_INSTALL_DIR picks
# where parcel.exe goes (default: %LOCALAPPDATA%\Programs\parcel\bin, added to the user PATH).
...
$Repo = 'griot-cloud/parcel'
$Bin = 'parcel'
$Target = 'x86_64-pc-windows-msvc'
$Version = if ($env:PARCEL_VERSION) { $env:PARCEL_VERSION } else { 'latest' }
$Dir = if ($env:PARCEL_INSTALL_DIR) { $env:PARCEL_INSTALL_DIR } else { Join-Path $env:LOCALAPPDATA "Programs\$Bin\bin" }
