...
async fn user_functions_end_to_end() {
    ...
    let bundle = engine.bundle("kplc/tokens").unwrap();
    assert_eq!(bundle.functions.len(), 3);
    parcel_engine::bundle::Bundle::from_json(&bundle.to_json().unwrap())
        .unwrap()
        .verify()
        .unwrap();

    // A reopened workspace reloads the functions from storage.
    let (reopened, _) = Engine::open(dir.path()).unwrap();
    assert_eq!(reopened.functions().len(), 3);
}
